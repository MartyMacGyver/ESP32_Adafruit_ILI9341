...
static void spi_intr_func_set(void * isr, spi_num_e spiNum)
{
     if (SpiNum_SPI1 == spiNum) {
        intr_matrix_set(0, ETS_SPI1_INTR_SOURCE, 13);
        xt_set_interrupt_handler(13, (isr), NULL);
        ESP_INTR_ENABLE(13);
    } else if (SpiNum_SPI2 == spiNum) {
        intr_matrix_set(0,ETS_SPI2_INTR_SOURCE, 17);
        xt_set_interrupt_handler(17, (isr), NULL);
        ESP_INTR_ENABLE(17);
    } else if (SpiNum_SPI3 == spiNum) {
        intr_matrix_set(0,ETS_SPI3_INTR_SOURCE, 18);
        xt_set_interrupt_handler(18, (isr), NULL);
        ESP_INTR_ENABLE(18);
    } else {
        // To do nothing
    }
